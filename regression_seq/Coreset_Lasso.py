...
def coreset_lr(X, y, h, lamda):
    """
       ...
       """
    f = np.power(np.dot(X,h)-y,2)+np.ones((X.shape[0],1))*np.sum(lamda*np.abs(h),0)/X.shape[0]
    X = np.hstack((X, y))
    L_max = np.max(f, 0)
...
def FGD(max_iter, X,y,weight,h,lamda,lr=0.01):
    ...
    also PGD of FGD
    """
    threshold = lr * lamda /(np.sum(weight)*100)
    for iter in range(max_iter):
        gradient_f = (np.sum(weight*(2*(X*(np.dot(X, h) - y))),0)[:, np.newaxis])/np.sum(weight)
        h -=lr*gradient_f
        index_i = np.array(np.where((np.abs(h) <= threshold)))[0]
...
def l2_loss_test_reg(X_test, y_test, beta, lamda):
    loss = np.sum(np.power(np.dot(X_test, beta) - y_test, 2), 0) / X_test.shape[0]+np.sum(lamda/9*np.abs(beta),0)/X_test.shape[0]
    return float(loss)
...
def main():
    ...
    for d in [dimension]:
        ...
        scio.savemat( 'coreset_syn_lasso{}_ep{}_iter_{}_Samplemin_{}_N_{}_d_{}.mat'.format(learning_rate, epsilon, epoch_max,
                                                                                     n_range[0], Num, dimension),
